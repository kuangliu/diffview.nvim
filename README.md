...
staged with `git add --all` before committing). If DeepSeek is configured,
the message is generated by the API first and pre-filled in the prompt.
...
```lua
{
  ...
  config = function()
    require('diffview').setup({
      -- Optional: use DeepSeek to generate the commit message opened by `c`.
      deepseek = {
        api_key = 'sk-...',
        model = 'deepseek-chat',
...
## DeepSeek commit messages

Configure the API key and model in `setup()` (or set `DEEPSEEK_API_KEY` as a
fallback for the key). When `c` is pressed, the plugin sends the working tree
diff, `git status`, and the contents of untracked text files to DeepSeek's
`chat/completions` endpoint, waits for the reply, and then opens the commit
prompt with the generated message already filled in. Markdown fences and
wrapper labels in the response are stripped automatically. Reasoning models
that exhaust the token budget before producing the final message are retried
automatically with a larger budget. If the request fails or no key is
configured, the prompt opens empty so the commit can still be entered
manually.
...
| `c`                | From the diff view or nvim-tree: commit all modified files; with DeepSeek configured, the commit-message prompt is pre-filled by AI; closes the view when all changes are committed. |
